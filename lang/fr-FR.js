// prettier-ignore
export default {
  close: "Fermer",
  home__projects: "Mes projets",
  home__contact: "Contact",
  home__github: "Github",
  home__resume: "CV",
  home__title: "Développeur",
  show__demo: "Voir la démo",
  see__more: "Voir",
  see__recette: "Voir l'environnement de recette",
  show__playstore: "Google Play Store",
  form__success: "Le message a été envoyé.",
  form__error: "Une erreur est survenue.",
  form__full_name: "Nom prénom",
  form__email_address: "Email",
  form__message: "Message",
  form__submit: "Envoyer",
  form__required_field: "Champ requis",
  form__invalid_email: "Email invalide",
  form__too_long: "Le message est trop long",
  form__or: "OU",
  form__email_me: "Envoyez-moi un mail à",
  demo__add_path: "Créer un trajet",
  demo__fill_stops: "Préciser les arrêts de départ / arrivée",
  demo__choose_path: "Choisir un trajet pour le widget",
  demo__add_widget: "Ajouter le widget à l'écran d'accueil",
  demo__update_times: "Actualiser les temps de passage",
  demo__switch_directions: "Changer de direction",
  demo__add_new_path: "Ajouter un nouveau trajet",
  demo__change_path: "Changer de trajet pour le widget",
  demo__delete_path: "Supprimer un trajet / annuler sa suppression",
  demo__display_wines: "Afficher la liste des vins",
  demo__add_wine: "Ajouter un vin",
  demo__add_bottle: "Entrer une bouteille en stock",
  demo__display_bottle: "Afficher une bouteille",
  demo__search_bottle: "Rechercher une bouteille",
  demo__consume_bottle: "Consommer une bouteille",
  demo__show_history: "Consulter l'historique",
  demo__show_stats: "Consulter les statistiques",
  demo__delete_wine: "Supprimer un vin",
  demo__no_video: "Impossible de lire la vidéo",
  multi__search__story: "Un package NPM qui permet de faire des recherches multi critères dans une grappe d'objet venant d'une API. Registre NPM: https://www.npmjs.com/package/multi-search-pipe",
  grtw__subtitle: "Horaires en temps réel des bus de Besançon (25)",
  synapp__story: "Application web permettant la gestion d'une base de données d'adhérent, et l'organisation de la vie autour de l'association. Afin de tester l'application, vous pouvez vous connecter en tant qu'administrateur via le user test@gmail.com et le mot de passe test, et utiliser les mêmes informations pour vous connecter à la partie 'Adhérent'. Bonne visite !",
  synapp_subtitle: "CRM visant à aider les administrateurs d'association caritative à but non lucratif",
  cavity_backend__story: "Une API REST utilisée pour stocker les données des utilisateurs de l'application mobile Cavity et restaurer des sauvegardes de cave à vin sur différents appareils.",
  cavity_backend__subtitle: "Serveur de sauvegarde de données utilisateur",
};
